...
class CardgeneratorValidationException implements Exception {
  final List<String> errors;

  CardgeneratorValidationException(this.errors);

  @override
  String toString() => 'CardgeneratorValidationException: ${errors.join("; ")}';
}
...
/// Card Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/cardgenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [brand] (required) - The brand of the card to generate
/// * [count] - The number of test card numbers to generate [min: 1, max: 100]
class CardgeneratorClient {
  ...
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CardgeneratorValidationException(errors);
    }
  }

  /// Execute a request to the Card Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<CardgeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CardgeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CardgeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CardgeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CardgeneratorException('Resource not found');
      } else {
        throw CardgeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CardgeneratorException) rethrow;
      throw CardgeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Card Generator API client.
class CardgeneratorException implements Exception {
  final String message;

  CardgeneratorException(this.message);

  @override
  String toString() => 'CardgeneratorException: $message';
}
